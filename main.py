...
import httpx
from fastapi import FastAPI, File, UploadFile, HTTPException, Depends, Cookie, Response
...
async def google_callback(code: str = "", state: str = "", error: str = ""):
    ...
    if not email.endswith(f"@{ALLOWED_DOMAIN}"):
        return RedirectResponse(f"/login?error=domain")
...
async def _load_notion_cache():
    ...
    try:
        ...
        print(f"[Notion] 제품 {len(products)}개 캐시 완료")
    except Exception as e:
        print(f"[Notion] 로딩 오류: {e}")
  ...
  "description": "제품 특징 한국어 1문장"
    ...
    try:
        ...
        if hasattr(gemini_client, 'models'):
            ...
            resp = gemini_client.generate_content([img_part, prompt])
            text = resp.text
...
async def update_notion_name(body: dict, email: str = Depends(_require_auth)):
    ...
    name_prop = os.getenv("NOTION_NAME_PROPERTY", "제품명")
    payload = {"properties": {name_prop: {"title": [{"text": {"content": new_name}}]}}}
